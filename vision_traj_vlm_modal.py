...
class Vision_Trajectory_Model(nn.Module):
    ...
    def mdn_loss(self, y, y_prime):
        ...
        soft_target = F.softmax(-l2_norm / self.args.pred_len, dim=0).t().detach() # [N, F]
        # cls_loss += self.cls_loss(out_pi, soft_target)
        loss_cls = F.kl_div(F.log_softmax(out_pi, dim=-1), soft_target, reduction='batchmean')
        loss = reg_loss + loss_cls
        #best ADE
        sample_k = out_mu[best_mode, torch.arange(batch_size)] #[N, H, 2]
        full_pre_tra.append(sample_k)
        softmax_pi = F.softmax(out_pi, dim=-1)
        best_pi = softmax_pi[torch.arange(batch_size), best_mode]
        # best FDE
