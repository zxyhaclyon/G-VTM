...
normalize_dict = {'location1':{'x_min':-35.1624, 'x_max':31.8797, 'y_min':-35.1927, 'y_max':37.9279}, \
                  'location2':{'x_min':-40.3597, 'x_max':51.2069, 'y_min':-27.7715, 'y_max':27.1802},
                  'location4':{'x_min':-69.0464, 'x_max':67.2594, 'y_min':-43.7174, 'y_max':68.8208}}
grid_dict={
    'location1':{'x_min':-42, 'x_max':38, 'y_min':-40, 'y_max': 40}, \
    'location2':{'x_min':-42, 'x_max':54, 'y_min':-32, 'y_max': 32},
...
def remove_still_vehicle(tracks, tracks_meta, exceptions=()):
    ...
    for ti in track_ids:
        if ti in exceptions:
            continue
        v_type = tracks_meta[tracks_meta.trackId == ti]['class'].iloc[0]
...
if __name__ == "__main__":
    ...
    if city == 'location1':
        rec_ids = ['0' + str(f) if len(str(f)) < 2 else str(f) for f in range(7, 18)]
        window_stride = 6
    ...
    np.random.seed(1234)
    for r_id in rec_ids:
        train_data, train_edge_feat, train_mask, train_target = [], [], [], []
        valid_data, valid_edge_feat, valid_mask, valid_target = [], [], [], []
        test_data, test_edge_feat, test_mask, test_target = [], [], [], []
        if r_id in ('00', '01', '02', '03', '04', '05', '06'):
            p0 = (117.65284072194481, -71.20122171472758)
        elif r_id in ('07', '08', '09', '10', '11', '12', '13', '14', '15', '16', '17'):
            p0 = (54.029258714756, -32.14253800176084)
