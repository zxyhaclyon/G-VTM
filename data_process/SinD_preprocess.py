...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_read", default=f'{read_dir}/SinD_data/tianjin', type=str, help="data read path")
    ...
    for sub_folder in sub_folders:
        ...
        # 一个城市的一段采集时间代表一个文件
        if not os.path.exists(f'{data_root}/training/observation/{record_id}'):
            os.mkdir(f'{data_root}/training/observation/{record_id}')
            os.mkdir(f'{data_root}/training/target/{record_id}')
            os.mkdir(f'{data_root}/validation/observation/{record_id}')
            os.mkdir(f'{data_root}/validation/target/{record_id}')
            os.mkdir(f'{data_root}/testing/observation/{record_id}')
            os.mkdir(f'{data_root}/testing/target/{record_id}')
        np.savez(f'{data_root}/training/observation/{record_id}/dat.npz', input = np.stack(train_data, axis=0))
        np.savez(f'{data_root}/training/observation/{record_id}/mask.npz', mask = np.stack(train_mask, axis=0))
        np.savez(f'{data_root}/training/observation/{record_id}/edge_feat.npz', edge = np.stack(train_edge_feat, axis=0))
        np.savez(f'{data_root}/training/target/{record_id}/label.npz', label = np.stack(train_target, axis=0))
        
        np.savez(f'{data_root}/validation/observation/{record_id}/dat.npz', input = np.stack(valid_data, axis=0))
        np.savez(f'{data_root}/validation/observation/{record_id}/mask.npz', mask = np.stack(valid_mask, axis=0))
        np.savez(f'{data_root}/validation/observation/{record_id}/edge_feat.npz', edge = np.stack(valid_edge_feat, axis=0))
        np.savez(f'{data_root}/validation/target/{record_id}/label.npz', label = np.stack(valid_target, axis=0))

        np.savez(f'{data_root}/testing/observation/{record_id}/dat.npz', input = np.stack(test_data, axis=0))
        np.savez(f'{data_root}/testing/observation/{record_id}/mask.npz', mask = np.stack(test_mask, axis=0))
        np.savez(f'{data_root}/testing/observation/{record_id}/edge_feat.npz', edge = np.stack(test_edge_feat, axis=0))
        np.savez(f'{data_root}/testing/target/{record_id}/label.npz', label = np.stack(test_target, axis=0))
